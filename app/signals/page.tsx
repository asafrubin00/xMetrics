  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-7xl px-5 py-8 sm:px-8 lg:px-12 lg:py-12">
      ...
      <div className="grid gap-10 py-10 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <section aria-labelledby="findings-heading"><p className="text-xs font-semibold uppercase tracking-[0.22em] text-gold-400">Findings</p><h2 id="findings-heading" className="mt-2 font-display text-3xl text-cream-50">What stands out</h2><div className="mt-6 space-y-4">{session.signals.length === 0 ? <p className="rounded-xl border border-navy-700 bg-navy-900 p-5 text-sm leading-6 text-cream-300">No threshold-level concentrations, vacuums or polarities appear in this team.</p> : session.signals.map((signal, index) => <article data-testid="signal-card" key={`${signal.kind}-${signal.traitId}-${signal.memberIds.join("-")}-${index}`} className="rounded-xl border border-navy-700 bg-navy-900 p-5"><span className="text-[10px] font-bold uppercase tracking-[0.18em] text-gold-400">{signalLabels[signal.kind]}</span><p className="mt-3 text-sm leading-6 text-cream-100">{signal.narrative}</p></article>)}</div></section>
      </div>
