...
function ShortlistStartInner() {
  const searchParams = useSearchParams();
  const longList = searchParams.get("ll") ?? "";
      ...
      <header className="mx-auto flex w-full max-w-5xl shrink-0 items-start justify-between gap-5">
        <div>
          ...
          <p className="mt-2 text-sm text-cream-300">
            Hand it to the agent, or seat the room yourself.
          </p>
        </div>
        ...
        <div className="grid w-full gap-5 text-left sm:grid-cols-2">
          ...
          >
            ...
            <span className="mt-3 block max-w-sm text-sm leading-6 text-cream-300">
              Brief the agent and let it rank and seat the strongest finalists.
            </span>
          ...
          <Link
            href={`/search?ll=${longList}&mode=manual`}
          ...
          >
            <span className="font-display text-3xl text-cream-100">
              Build your shortlist manually
            </span>
            <span className="mt-3 block max-w-sm text-sm leading-6 text-cream-300">
              Choose the finalists yourself and watch the room dynamics form live.
            </span>
            <span className="mt-8 block text-xs font-semibold uppercase tracking-[0.2em] text-gold-400">
              Go manual →
            </span>
...
export default function ShortlistStartPage() {
  return (
